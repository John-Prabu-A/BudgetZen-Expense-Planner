...
export const deleteAccount = async (id) => {
  try {
    console.log('🗑️ [deleteAccount] Deleting account:', id);
    
    // Step 1: Find all transfer records where this account is involved
    const { data: transferRecords, error: transferError } = await supabase
      .from('records')
      .select('id, transfer_group_id, type')
    ...
    console.log('📋 [deleteAccount] Found', transferRecords?.length || 0, 'transfer records');
    
    // Step 2: Delete transfer groups that involve this account
    // (transfers can't exist without both accounts)
    if (transferRecords && transferRecords.length > 0) {
      const transferGroupIds = [...new Set(transferRecords
      ...
      if (transferGroupIds.length > 0) {
        ...
        console.log('✅ [deleteAccount] Deleted', transferGroupIds.length, 'transfer groups');
      }
    }
    
    // Step 3: Find and delete regular records (income/expense) from this account
    ...
    if (regularRecords && regularRecords.length > 0) {
      const { error: deleteRegularError } = await supabase
        .from('records')
        .delete()
        .in('id', regularRecords.map(r => r.id));
      
      if (deleteRegularError) {
        throw new Error(deleteRegularError.message);
      }
      
      console.log('✅ [deleteAccount] Deleted', regularRecords.length, 'regular records');
    }
    
    // Step 4: Delete the account itself
...
export const deleteCategory = async (id) => {
  try {
    console.log('🗑️ [deleteCategory] Deleting category:', id);
    
    // Step 1: Find all records with this category
    const { data: recordsWithCategory, error: fetchError } = await supabase
      .from('records')
      .select('id')
      .eq('category_id', id);
    ...
    console.log('📋 [deleteCategory] Found', recordsWithCategory?.length || 0, 'records with this category');
    
    // Step 2: Update all records to have NULL category_id (deassign from category)
    if (recordsWithCategory && recordsWithCategory.length > 0) {
      const recordIds = recordsWithCategory.map(r => r.id);
      ...
      console.log('✅ [deleteCategory] Reassigned', recordIds.length, 'records - category_id set to NULL');
    }
...
export const deleteAllUserData = async (userId) => {
  ...
  try {
    console.log('🗑️ [deleteAllUserData] Starting full data wipe for user:', userId);
    
    // Ordered deletion to prevent foreign key constraint issues
    // 1. Delete all records
    const { error: recordsError } = await supabase.from('records').delete().eq('user_id', userId);
    if (recordsError) throw new Error(recordsError.message);
    console.log('✅ [deleteAllUserData] Records deleted');
    ...
    console.log('✅ [deleteAllUserData] Budgets deleted');
    
    // 3. Delete all accounts
    const { error: accountsError } = await supabase.from('accounts').delete().eq('user_id', userId);
    if (accountsError) throw new Error(accountsError.message);
    console.log('✅ [deleteAllUserData] Accounts deleted');
    
    // 4. Delete all custom categories
    const { error: categoriesError } = await supabase.from('categories').delete().eq('user_id', userId);
    if (categoriesError) throw new Error(categoriesError.message);
    console.log('✅ [deleteAllUserData] Categories deleted');
    
    console.log('✨ [deleteAllUserData] Full data wipe complete');
    return true;
  } catch (error) {
    console.error('❌ [deleteAllUserData] Critical error during wipe:', error.message);
    throw new Error(error.message);
  }
};
